...
# 3) Supports .mp3 songs that includes Unicode paths (not orignaly supported by the .m3u format) #
#                                                                                                #
# Playlist notes:                                                                                #
# .m3u playlists are partially supported by the sansa player, and the songs are                  #
# shown according to thir PATH NAMES, and NOT their "title" (like shown in the song list).       #
...
# 2.1) In case the "comment" tag is present, it will be used as the actuall name                 #
...
####################################################################################
# NOTE: additional absolute paths can be added manualy to the "my_playlists" list. #
...
def updateFileName(abspath, name) :
	...
	if need_convert and not can_convert :
		print 'Cant handle unicode named string - not an .mp3 song - %s' % repr(abspath)
		return None
...
def getPlaylistSongs(playlist) :
	...
	for line in codecs.open(playlist, encoding='utf-8').readlines() :
		...
		if trimmed.startswith(song_prefix) :
			...
			file_name = updateFileName(abs_path, file_name)
